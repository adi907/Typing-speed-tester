...
function invertColor(itema,itemr){
...
const retrieve_Leaderboard='https://prod-86.westus.logic.azure.com:443/workflows/39ee64ffd8104e259712578f6604949b/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=kFo6bBQUeXHAbXypGDqD_iMprntx2jAKBpm4zYZNN3E';

const retrieve_HighScore='https://prod-90.westus.logic.azure.com:443/workflows/ddc1b0e0a2c8476a9f404a4390b76039/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=8zCKw1tAVHoVVZdoF2B752HSozVCyr1TA-sus68wbIo';
...
function dataSubmit(wpm,acc){
    ...
    fetch(create_recordApi,options)
...
showLeaderboard(); //called to get a list even if you dont want to create a record
showHighScore();

async function showLeaderboard(){
    const response=await fetch(retrieve_Leaderboard);
  ...
  for(var i=0;i<myJson.payload.length;i++){
      let item=myJson.payload[i];
        ...
        <th scope="row">${i+1}</th>
        <td id="username${i+1}">${item.crdeb_username}</td>
        <td id="wpm${i+1}">${item.crdeb_wpm}</td>
        <td id="acc${i+1}">${item.crdeb_accuracy}</td>
        </tr>`;
...
async function showHighScore(){
  let em=email;
    ...
    const response=await fetch(retrieve_HighScore,options)
  ...
  console.log(myJson2.payload.length);
  

  for(var i=0;i<myJson2.payload.length;i++){
      let item=myJson2.payload[i];
  ...
  document.getElementById('highscore').innerHTML=tab;
}
